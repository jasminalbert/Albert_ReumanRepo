...
G3) To define the parameter space for which species can coexist in our model. 
...
To further explore the model and acheive G3 from the introduction i.e., exploring the parameter space for which coexistence is possible, we then plotted the invasion growth rate, $r_{i/i}-q_{12}r_{i/j}$ and the invasion growth rate [IGR] without ATA contributribution  $r_{i/i}-q_{12}r_{i/j} - \Delta_i^{[EC]}$ against the difference between species, $\mu_1-\mu_2$, 

Because $\mu$ is the only unique species parameter and the difference, $\mu_1-\mu_2$, is the only difference between the two species in this model, we henceforward call thus difference the trait difference between species. Letting the trait $\mu$ vary within a one dimensional space, this space is then a trait space, and the difference, $\mu_1-\mu_2$, is then a subset of that trait space. We then define $\psi_{ATA}$ to be the subset of trait space that allows a postive IGR when ATA is included and $\psi_{~ATA}$ to be the subset of trait space that allows a postive IGR when ATA is not included. We then use symmetric difference $\psi_{ATA} \Delta \psi_{~ATA}$ to make the following conslusions. If the length of $\psi_{ATA} > the length of $\psi_{~ATA}$ the $\psi_{ATA} \Delta \psi_{~ATA}$ consitutes ATA rescue and the degree of species trait difference that allows ATA rescue to act, hence, the ATA rescue trait space. If the length of $\psi_{ATA} < the length of $\psi_{~ATA}$ the $\psi_{ATA} \Delta \psi_{~ATA}$ consitutes ATA exclusion and the degree of species trait difference that allows ATA exclusion to act, hence, the ATA exclusion trait space.

In order to find the ATA rescue trait space and ATA exclusion trait space, we plotted the trait space subset $\mu_1-\mu_2$ against IGR and IGR without ATA for different values of $\sigma$ and $\delta$, the shared, non-unique species parameters. We selected $\delta=0.5, 0.8, 1.0$ and $\sigma=2.0,4.0,5.0, 6.0$. The spaces in between the lines where they intersect the x-axis at y=0, indicate either an ATA rescue trait space or an ATA exclusion trait space. If the line plotting IGR is to the left of the line plotting IGR without ATA, then this space is an ATA rescue trait space. If the line plotting IGR is to the right of the line plotting IGR without ATA, then this space is an ATA exclusion trait space. 
...
To explore the relative magnitudes of the contributions to coexistence that come from ATA versus other mechanisms in our simple two-species lottery model (G2), we plotted the decomposing mechanisms against increasing $\sigma$ for various values of $\mu_1 - \mu_2$ and $\delta$ (Figure 4). In almost all cases, aside from when $\delta=1$, the contribution of ATA is less than the contribution of storage effects without ATA, and the former decreases with $\sigma$ while the latter increases with $\sigma$. We see the opposite when $\delta=1$, where the contribution of ATA and greater than the contribution of storage effect without ATA and the trends are reversed. 
%outline of main resutls
Using our simple two-species lottery model, ATA made a difference for coexistence. Depending on death rate, variance, and difference between species, ATA can either impede or facilitate coexistence. In some cases, the contribution of ATA is the mechanism solely responsible for a positive (or negative) IGR, and the IGR without ATA is negative (or positive). In almost all cases we tested, ATA contributed negatively to coexistence expected for when delta = 1, annual species. Of the annual species cases, only when the species were the same or extremely close to being the same, did ATA have a rescue effect on storage effects and coexistence. For other annual species cases, as the species became moree different, IGR was more negative and the contribution of ATA and storage effect in general became more negligible. 

We found the range of species difference, where ATA either allowed the rescue of IGR or ATA imperiled IGR. %are we calling this 'differential coexistence'?

In general, the direction of the ATA did not show any differences (other than small differences in very specific cases) for coexistence

...
To further explore the model and acheive G3 from the introduction i.e., defining the ATA rescue or ATA exlusion subset of trait space for which species will coexist or become competitively excluded in our model, we plotted the invasion growth rate, $r_{i/i}-q_{12}r_{i/j}$ and the invasion growth rate [IGR] without ATA contributribution  $r_{i/i}-q_{12}r_{i/j} - \Delta_i^{[EC]}$ against the difference between species, $\mu_1-\mu_2$. The same mathematical simplifications described above still hold, such that $\mu1$ and $\mu2$ are the only ways that the species differ and computations for coexistence in the lottery model only depend on the difference, $\mu_1-\mu_2$. We henceforward call this difference the trait difference between species. Letting the trait $\mu$ vary within a one dimensional space, this space is then a trait space. Letting there me two points $\mu_1$ and $\mu_2$ within that trait space, then the difference, $\mu_1-\mu_2$, is a subset of that trait space. We then define $\psi_{ATA}$ to be the subset of trait space that allows a postive IGR when ATA is included and $\psi_{~ATA}$ to be the subset of trait space that allows a postive IGR when ATA is not included. We then use symmetric difference $\psi_{ATA} \Delta \psi_{~ATA}$ to make the following conslusions. If the length/range? of $\psi_{ATA}$ > the length/range? of $\psi_{~ATA}$ then $\psi_{ATA} \Delta \psi_{~ATA}$ consitutes ATA rescue and the degree of species trait difference that allows ATA rescue to act, hence, the ATA rescue trait space. If the length of $\psi_{ATA}$ < the length/range? of $\psi_{~ATA}$ then $\psi_{ATA} \Delta \psi_{~ATA}$ consitutes ATA exclusion and the degree of species trait difference that allows ATA exclusion to act, hence, the ATA exclusion trait space. We highlight and color code these areas in our plots. We made the plots for values of $\delta=0.5, 0.8, 1.0$ and $\sigma=2.0,4.0,5.0, 6.0$. The spaces in between the lines where they intersect the x-axis at y=0, indicate either an ATA rescue trait space or an ATA exclusion trait space. If the line plotting IGR is to the left of the line plotting IGR without ATA, then this space is an ATA rescue trait space. If the line plotting IGR is to the right of the line plotting IGR without ATA, then this space is an ATA exclusion trait space. 

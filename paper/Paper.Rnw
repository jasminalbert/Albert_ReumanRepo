...
For fixed values of $\sigma$ and $\delta$, we were interested in the values of $\mu_1-\mu_2$ for which one of these
...
i.e., $\mu_1 - \mu_2$ for which coexistence occurs in the absence, but not the presence, of ATAs.

...
%***DAN: We need to select 4 sensible values, including \delta=1, and put the ones we select here
and $\mu_1-\mu_2=A,B,C,D$,
%***DAN: We need to select 3-4 sensible values and put the ones we select here

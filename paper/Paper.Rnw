...
for left-tail associated noise. For each combination of $\delta=W, X, Y, Z$
%***DAN: Jasmin, please select 4 sensible values, including \delta=1, and put the ones you select here
and $\mu_1-\mu_2=A,B,C,D$,
%***DAN: Jasmin, please select 3-4 sensible values and put the ones you select here
...
plots to assess whether and to what extent ATA effects on coexistence were meaningfully large for the lottery model.
%***DAN: Jasmin, if you select 4 values for mu1-mu2 and 4 for delta, then Fig. 3 is going to have 16 panels arranged in a 
%4 by 4 grid, each of which will plot all the components against sigma. You'll need to use different colors and/or line
%types for the components and have a legend.
...
SI section X.
%***DAN: Dan to add the reference to the right sup mat section once things get sufficiently organized in sup mat to know what section.

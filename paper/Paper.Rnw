...
The realiance of storage effects on the relationship between environment and competition indicate that storage effects involve environment-coexistence associations, and hence there is potential for ATAs to influence storage effects. 
In addition to EC covariance, functioning storage effects also require that conspecific populations respond differently to environmental fluctuations, and different life stages must also vary in responses. 
...
Conversely, competition is more negatively related to environmental response for the low density species, leading to large fluctuations in population growth rate in both directions. 
Although, this may appear like a scenario for extiction, it can, in fact, be at an advantage due to differences in between life stages, which buffer the species against extinction by limiting negative growth rates, but not positive ones. 

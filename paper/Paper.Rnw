...
G2) To explore using a simple model the relative magnitude of the contributions of coexistence that come
from ATA compared to other mechanisms.

\section{Theory}\label{section:theory}

In this section we pursue goal G1 from the Introduction: to augment the MTC to show how ATA
plays a role in species coexistence.
%***DAN: make sure you define MTC and ATA once (probably in the Intro) and then use the abbreviations onle after that.
For pedgogical clarity, we develop each step of the theory in general and for a simple classical model 
(the lottery model), as an example. 
...
So $\Delta_i^{(EC)}$ for the lottery model can be computed by using simulations \citep{Ellner_2016}
...
The term $\Delta_i^{[EC]}$ is the component due to $EC$ correlation \emph{per se}, after 
...
$\var(e_i)=\var(c_{i \bs i})=1$, and we denote $\cov(e_i,c_{i \bs i})$ by $\tilde{\rho}_i$. We define 
...
component $\olrn_{i \bs i}$ of (\ref{eq:storage_effects_further}) for the lottery model using the simulation 
approach of \cite{Ellner_2016,Ellner_2019}. Note that $E_j$ and $C_{j \bs i}$ are linearly related for the lottery model,
so $E_j^\nat=E_j$ and $C_{j \bs i}^\nat = C_{j \bs i}$. Thus $\olr_{j \bs i}=\olrn_{j \bs i}$.
...
for which asymmetries of tail association are relevant. Mathematical details are given in SI section 
\ref{SI-SIsect:other_mechanisms}, but we here provide a conceptual understanding. \cite{Ellner_2016} and
\cite{Ellner_2019} provide a decomposition of $\olr_{i \bs i} -q_{ij}\olr_{i \bs i}$ which is similar to and motivated by the 
...
$\Delta_i^E$ is a contribution of environmental variation;
$\Delta_i^C$ is a contribution of variation in competition;
$\Delta_i^{(E\shp C)}$ is a contribution of having simultaneous variation in both environment and competition, but not including the effects 
of covariation in these quantities; and
$\Delta_i^{(EC)}$ is a contribution of $EC$ covariation -- storage effects. 
Only storage effects involve $EC$ covariation,
so the only mechanism of coexistence influenced by ATA is storage effects. Pairing our decomposition 
(\ref{eq:storage_effects_further1})-(\ref{eq:storage_effects_further}) with the 
previous MTC decomposition (\ref{eq:previous_decomp}) gives the extension of the MTC which was goal G1 of the Introduction. 

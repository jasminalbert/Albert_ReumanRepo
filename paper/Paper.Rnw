...
Here $\E(\cdot)$ denotes expected value just as does an overbar, but we use $\E$ for longer expressions for which
an overbar would be cumbersome. Likewise, 
$\olr_{j \bs i} = \E[\ln(1-\delta+\delta B_j/B_j)]=\E[\ln(1-\delta+E_j/C_{j \bs i})]$,
though we already knew this equals $0$. An expression for $\olrs_{i \bs i}$ is then obtained from the 
above expression for $\olr_{i \bs i}$ by replacing the bivariate random variable $(E_i,C_{i \bs i})=(B_i,B_j/\delta)$
by a random variable with the same marginals, but independent components, $(B_i^\shp,B_j^\shp /\delta)$.
Thus $\olrs_{i \bs i} = \E[\ln(1-\delta+\delta B_i^\shp/B_j^\shp)]$. Likewise, a formula for 
$\olrs_{j \bs i}$ is available by replacing $(E_j,C_{j \bs i})$ in an analogous way. We use 
...
The superscript ``cps" stands for ``correlation
\emph{per se}'' because this component is due to $EC$ correlation \emph{per se}, after 
removing the effects of asymmetric tail associations.
The notation $\nat$ was chosen because this new symbol is part of the symbol $\shp$ (i.e., two of its four lines), 
corresponding to the removal of part of the relationship between $E$ and $C$ components. 
...
$\var(e_i)=\var(c_{i \bs i})=1$, and denote $\cov(e_i,c_{i \bs i})$ by $\tilde{\rho}_i$. We define 
$E_i^\nat = F_{E_i}^{-1} \circ \varphi(e_i)$
and $C_{i \bs i}^\nat=F_{C_{i \bs i}}^{-1} \circ \varphi(c_{i \bs i})$. The value of $\tilde{\rho}_i$ is then determined 
in one of 
...
$\tilde{\rho}_i=\rho_i$, which straightforwardly implies that $(E_i^\nat,C_{i \bs i}^\nat)$
can be written as $(\exp(\tilde{b}_i),\exp(\tilde{b}_j))$, where $(\tilde{b}_i,\tilde{b}_j)$
is bivariate normally distributed with mean $(\mu_i,\mu_j)$, $\var(\tilde{b}_i)=\var(\tilde{b}_j)=\sigma$,
and $\cov(\tilde{b}_i,\tilde{b}_j)=\cov(b_i,b_j)$. So $(\tilde{b}_i,\tilde{b}_j)$ has the same
...
components of (\ref{eq:storage_effects_further}) for the lottery model using the simulation 
approach of \cite{Ellner_2016,Ellner_2019}, though in the next section we describe some tricks
that make these computations efficient. 

The other mechanism in modern coexistence theory that apply in our modelling context (i.e., purely temporal variation)
...
for which asymmetries of tail association are relevant. Details are given in SI section 
\ref{SIsect:other_mechanisms}, and we return briefly to related ideas in the Discussion.
%***DAN: In the Discussion, I intend to talk about how possible changes in tail association 

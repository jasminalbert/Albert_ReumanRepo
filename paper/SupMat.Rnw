...
&= \frac{1}{(1-\delta)\exp(-(\mu_i-\mu_j))+\delta}.
\end{align}
...
%***DAN: fill in the value, 1000000 if you can swing it
...
$s_1$ is as above, and $s_3=\sd_k[\ln(1-\delta+\delta\exp(\tilde{b}_i^{(k)}-\tilde{b}_j^{(k)}))]/\sqrt{M}.$
...
\begin{enumerate}
\item Use $\epsilon_i^0=0$, as in (\ref{eq:decomp:baseline_j}), and this is the exact value.
...
\sd_k[\ln(1-\delta+\delta\exp(\sigma\sqrt{2}u^{(k)}))-\ln(1-\delta+\delta\exp(u^{(k)}-\sigma^2/2))-
\ln(1-\delta+\delta\exp(-\sigma u^{(k)}+\sigma^2/2))]/\sqrt{M}$.
\item Estimate
%***DAN: DAN, proceed from here
\end{enumerate}

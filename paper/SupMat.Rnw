...
%you want to be officially known as on scientific papers. Same change in main text, please.
...
We review the decompositions of \cite{Ellner_2016} and \cite{Ellner_2019}, which are similar to and motivated by the 
original decompositions of \cite{Chesson_1994}, and then we include our extension. 
...
The derivation also applies to provide 
\begin{equation}
...
species i is the weaker competitor, we have
\begin{equation}
...
The notation $\Delta I_i$ from the main text equals $\Delta_i^{(EC)}=\Delta_i^{[EC]}+\Delta_i^{[E\nat C]}$,
and $\Delta I_i^{(\text{ata})}$ from the main text is the same as $\Delta_i^{[E\nat C]}$ and $\Delta I_i^{(\text{cps})}$ 
from the main text is $\Delta_i^{[EC]}$ (see (\ref{MT-eq:storage_effects_further1}) and (\ref{MT-eq:storage_effects_further}) from the main text).

%***DAN: Next go through everything for the lottery model
%***DAN: Then argue that everything depends only on mu1-mu2
...
%***DAN: Sections below this point are from the earlier version/reasoning and may have to be edited and placed in other
%locations. This comment written 2021 06 22

\section{The Model}

%***DAN: Probably delete this section, since the model is now elaborated in the main text

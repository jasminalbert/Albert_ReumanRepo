...
for (g in 1:length(sigma)){
  for (m in 1:nrow(mu)){
    ...
    for (d in 1:length(delta)){
      ...
      #DeltaI (6)
      #left
      DeltaIhat_l <- l$rbar_hat - sharp1$rbar_hat + sharp2$rbar_hat
      se_DeltaIhat_l <- l$se_r + sharp1$se_r + sharp2$se_r
      
      #right
      DeltaIhat_r <- r$rbar_hat - sharp1$rbar_hat + sharp2$rbar_hat
      se_DeltaIhat_r <- r$se_r + sharp1$se_r + sharp2$se_r
      
      #symmetric
      DeltaIhat_s <- s$rbar_hat - sharp1$rbar_hat + sharp2$rbar_hat
      ...
      params <- rbind(params, cbind(sigma[g],mu[m,1],mu[m,2],delta[d]))
    }
  }
}
res <- list(params=params[-1,], invPrb=invPrb[-1,], invE=invE[-1,], rbar=rbar[-1,], se_rbar=se_rbar[-1,], sd_r=sd_r[-1,], DeltaI=DeltaI[-1,], se_DeltaI=se_DeltaI[-1,], epECdot=epECdot[-1,])
...
saveRDS(res, paste(resloc,"rbar_DeltaI.RDS", sep=""))
